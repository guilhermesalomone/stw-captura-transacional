...
//	private final ProducerService produtor;
	...
	public void enviarRequest(final String requestUrl, final String sessionId, final String method,
			final String clientIP, final String clientHost, final String parametros) {
...
//			}
	...
	public void enviarResponse(final String requestUrl, final String sessionId, final String method,
		...
		try {
			...
			final StwMonitorResponseDto response = StwMonitorResponseDto.builder().withHostName(properties.getName())
					.withApiVersion(properties.getApiVersion()).withMethod(method).withStatus(httpStatus)
					.withPathServico(requestUrl).withSessionId(sessionId).build();
